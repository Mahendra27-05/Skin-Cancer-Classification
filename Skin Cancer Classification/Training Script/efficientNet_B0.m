...
%% STEP 4: Load Pretrained EfficientNet-b0
net = efficientnetb0;
lgraph = layerGraph(net);

% Display all layer names to identify correct ones
disp('EfficientNet-b0 Layer Names (last 6):');
layerNames = {lgraph.Layers.Name};
disp(layerNames(end-5:end)');  % Show last 6 layers
...
% Remove pretrained final layers (correct layer names for EfficientNet-b0)
try
    lgraph = removeLayers(lgraph, {'predictions','predictions_softmax','ClassificationLayer_predictions'});
catch
    % Try alternative: remove last 3 layers one by one
    lgraph = removeLayers(lgraph, layerNames{end});
    lgraph = removeLayers(lgraph, layerNames{end-1});
    lgraph = removeLayers(lgraph, layerNames{end-2});
end

% Add new layers
lgraph = addLayers(lgraph, newLayers);

% Find the last pooling layer name
poolLayerIdx = find(contains(layerNames, 'pool', 'IgnoreCase', true));
if ~isempty(poolLayerIdx)
    lastPoolLayer = layerNames{poolLayerIdx(end)};
else
    lastPoolLayer = layerNames{end-3};  % Fallback
end

% Connect new layers
lgraph = connectLayers(lgraph, lastPoolLayer, 'fc_new');

fprintf('Using pretrained model: EfficientNet-b0\n');
fprintf('Connected to layer: %s\n', lastPoolLayer);
